...
from joint.models import JointNetwork, DualNetwork, DualNetworkShallow, DualNetworkShallower
from joint.models import ResNet6, ResNet8, ResNet16
from joint.models import LinearClassifier
from joint.datasets import DetectionDataset
from joint.models import NoiseEstNetwork

from typing import Dict, List
from joint.utils.losses import js_div_loss_2d
from joint.utils.losses import PuLoss, modified_pu_loss, PULoss, FocalLoss
...
class Denoiser(nn.Module):
    ...
    def _mask_mse_pipeline(self, data: List, **kwargs) -> Dict:
        ...
        if len(data) >= NoisyDataset.REFERENCE:
            ...
            if NoisyDataset.Metadata.MASK_COORDS in data[NoisyDataset.METADATA]:
                ...
                loss = joint.utils.n2v_loss.loss_mask_mse(mask_coords, cleaned, ref)
                loss = loss.view(loss.shape[0], -1).mean(1, keepdim=True)
                outputs[PipelineOutput.LOSS] = loss
    ...
    def config_name(self) -> str:
        return joint.cfg.config_name(self.cfg)
