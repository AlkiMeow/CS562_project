...
from joint.models import NoiseNetwork
from joint.models import JointNetwork
# from ssdn.denoiser import Denoiser
from joint.denoiser_v2 import Denoiser
from joint.utils import TrackedTime, Metric, MetricDict, separator
from joint.utils.data_format import DataFormat
from joint.utils.algorithms import non_maximum_suppression, match_coordinates, find_contamination
from joint.cfg import DEFAULT_RUN_DIR
from typing import Dict, Tuple, Union, Callable
from torch.utils.tensorboard import SummaryWriter
from collections import defaultdict
from joint.utils.crop import MyRandomCrop
import pandas as pd
import joint.utils.files as file_utils
import timeit
...
class DenoiserTrainer:
    ...
    def train(self):
        ...
        _ = self.writer
        joint.logging_helper.setup(self.run_dir_path, "log.txt")
        logger.info(joint.utils.separator())
    ...
    def learning_rate(self) -> float:
        ...
        if self.mode == "joint" or self.mode == "denoise":
            learning_rate = joint.utils.compute_ramped_lrate(
...
run_            outputs (Dict): Outputs to extract images from.
    ...
    def _save_image_outputs(
        ...
        def unpad_save(img: Tensor, target: Tensor, desc: str, find_contam = False):
            ...
            joint.utils.save_tensor_image(out[0], make_path(desc))
        ...
        def unpad_save_filt(img: Tensor, target: Tensor, desc: str):
            out = DetectionDataset.unpad(img, target, metadata, batch_index)
            joint.utils.save_tensor_image_filt(out[0], make_path(desc))
    ...
    def train_state_str(self) -> str:
        ...
        def eta_str():
            ...
            if isinstance(timings.get("eta", None), int):
                ...
                if eta < 1:
                    ...
                    return joint.utils.seconds_to_dhms(eta)
            else:
                return "???"
        ...
        if len(metric_str_list) > 0:
            summary += " | "
        summary += "[{} ~ ETA: {}]".format(
            joint.utils.seconds_to_dhms(total_train.total, trim=False), eta_str(),
        )
        return summary
    ...
    def calculate_psnr(
        ...
        #     clean = DetectionDataset.unpad(clean, metadata)
            ...
            psnrs = map(
                lambda x: joint.utils.calculate_psnr(*x, data_format=DataFormat.CHW),
                zipped,
            )
            ...
            psnr = joint.utils.calculate_psnr(outputs[output], clean)
        return psnr
    ...
    def config_name(self) -> str:
        ...
        config_name_lst = [joint.cfg.config_name(self.cfg), iter_str()]
    ...
    def set_train_data(self, path: str):
        ...
        joint.cfg.infer_datasets(self.cfg)
    ...
    def test_data(self) -> Tuple[DataLoader, DetectionDataset, Sampler]:
       ...
       # Load dataset
        ...
        sampler = FixedLengthSampler(
            dataset,
            num_samples=joint.cfg.test_length(cfg),
            shuffled=False,
        )
        dataloader = DataLoader(
            dataset,
